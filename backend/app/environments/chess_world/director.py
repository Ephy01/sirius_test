...
class FamilySettings:
    """Organizer-controlled bounds for one Chess World task family."""
...
def _stable_rank(*, seed: int, phase: str, family: str, turn: int) -> int:
    payload = f"{DIRECTOR_VERSION}:{seed}:{phase}:{turn}:{family}".encode("utf-8")
...
def _exposure_count(
    ...
    family_history = _history_for_family(history, family)
    if family != PENULTIMA_FAMILY:
        return len(family_history)
    # Three linked goals form one Penultima scene for route balancing, while
    # every goal still contributes adaptation evidence.
    return sum(
...
def decide_next_task(
    *,
    ...
    """Choose the next Chess World task from completed-task history.

    Precedence is intentional:

    1. an unfinished Penultima chapter stays locked to Penultima;
    ...
    if (
        latest is not None
        and latest.family == PENULTIMA_FAMILY
        ...
        reason = (
            "penultima_chapter_remediation"
            if phase == DirectorPhase.REMEDIATION
            else "penultima_chapter_continuation"
        )
        return _decision(
    ...
    counts = {
        family: _exposure_count(relevant_history, family)
